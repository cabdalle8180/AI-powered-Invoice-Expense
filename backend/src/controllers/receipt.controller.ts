...
const buildReceiptFilter = (
  req: AuthRequest
): { filter: Record<string, unknown>; denied: boolean } => {
...
const formatReceipt = (exp: InstanceType<typeof Expense>) => {
  ...
  return {
    ...
    category: exp.category
      ...
      : "Other",
    amount: exp.amount,
    status: isPending ? ("Pending" as const) : ("Reviewed" as const),
    notes: exp.notes,
    createdByName:
      typeof exp.createdById === "object" &&
      exp.createdById &&
      "name" in exp.createdById
        ? String((exp.createdById as { name?: string }).name)
        : "Staff",
  };
};
  ...
  try {
    ...
    if (search) {
      ...
      if (filter.$or && statusFilter === "pending") {
        filter.$and = [{ $or: filter.$or as Record<string, unknown>[] }, searchClause];
        delete filter.$or;
      } else {
    ...
    const totalAmountYTD = statsExpenses.reduce(
      (sum, e) => sum + (e.amount || 0),
      0
    );
    const pendingReviewCount = statsExpenses.filter(
      (e) => !e.vendor || e.category === "other"
    ).length;
...
export const createReceipt = async (
  ...
  try {
    ...
    const expense = await Expense.create({
      ...
      category: (category?.toLowerCase() || "other") as ExpenseCategory,
...
export const getReceiptById = async (
  ...
  try {
    ...
    const expense = await Expense.findOne({ _id: id, ...filter }).populate(
      "createdById",
      "name email role"
    );
...
export const updateReceipt = async (
  ...
  try {
    ...
    const expense = await Expense.findOne({ _id: id, ...filter });
    ...
    if (category !== undefined) {
      expense.category = String(category).toLowerCase() as ExpenseCategory;
    }
...
export const deleteReceipt = async (
  ...
  try {
    ...
    if (!rolesMatch(role, "superAdmin", "owner", "accountant")) {
      res.status(403).json({
        success: false,
    ...
    const expense = await Expense.findOneAndDelete({ _id: id, ...filter });
