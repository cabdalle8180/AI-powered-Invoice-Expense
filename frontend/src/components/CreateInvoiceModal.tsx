...
import React, { useState, useEffect } from "react";
import { X, Plus, Trash2 } from "lucide-react";
import invoiceService, { type CreateInvoicePayload, type IInvoiceItem, type InvoiceStatus } from "../api/invoiceService";
import { getCustomers, type Customer } from "../api/customerService";
...
export const CreateInvoiceModal: React.FC<CreateInvoiceModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convert items quantity & price explicitly to numbers before sending to API
      const formattedPayload: CreateInvoicePayload = {
        ...formData,
      ...
      await invoiceService.createInvoice(formattedPayload);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error("Error creating invoice:", err);
      alert(err.response?.data?.message || "Cillad ayaa dhacday marka la abuurayay invoice-ka.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-xl border border-slate-100 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pt-2">
            <label className="block text-xs font-semibold text-slate-600 mb-2">Invoice Items</label>
            {formData.items.map((item, index) => (
              <div key={index} className="flex items-center gap-2 mb-2">
                <input
                  type="text"
