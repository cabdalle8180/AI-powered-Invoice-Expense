...
import React, { useState, useEffect } from "react";
import { X, Plus, Trash2 } from "lucide-react";
import invoiceService, { type Invoice, type UpdateInvoicePayload, type IInvoiceItem, type InvoiceStatus } from "../api/invoiceService";
import { getCustomers, type Customer } from "../api/customerService";
...
export const EditInvoiceModal: React.FC<EditInvoiceModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedPayload: UpdateInvoicePayload = {
        ...formData,
      ...
      await invoiceService.updateInvoice(invoice._id, formattedPayload);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error("Error updating invoice:", err);
